...
// import { useParams} from "react-router-dom";
  ...
  const fetchLatestSessionId = async () => { 
    try {
      const res = await API.get("/session/latest");   
    const curr_sessionId = JSON.stringify(res.data);
      console.log("latest session id from fetchLatestSessionId : " + curr_sessionId);

      setSession(res.data);
      return curr_sessionId;
    } catch (error) {
  ...
  //  const sessionId =  await fetchLatestSessionId();
   console.log("latest session id : " + session);
